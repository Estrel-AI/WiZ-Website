...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await listHierarchy(body);
    return ok("Hierarchy list fetched successfully", data);
  } catch (error) {
    return badRequest(
      error instanceof Error ? error.message : "Unable to list hierarchy rows.",
    );
  }
}
